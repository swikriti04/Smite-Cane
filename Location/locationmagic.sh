...
then
	...
	if [ "$inputargs" = "-install" ]; then
		...
		crontab -l > crontab_$(whoami).backup | 2>&1
		echo "Adding script to run every hour"
		($(crontab -l > file | 2>&1; echo "0 * * * * cd /usr/local/bin/ && ./locationmagic.sh -locate $platform $token" >> file | sort file | uniq file | crontab -))
		echo "All done!"
